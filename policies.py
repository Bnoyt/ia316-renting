...
class Thomson_SamplingPolicy(Policy):
    ...
    def get_action(self,context):
        ...
        rwrds=df.groupby(['bike_id']).apply(lambda x: (x.accepted*x.price_proposed).sum()).tolist()
...
class EGreedyPolicy(Policy):
    """
    Example de policy qui fait un epsilon greedy
...
class DeepPolicyMemory(Policy):
    """
        Deep learning policy with memory
    """
    ...
    def __init__(self,params,seed):
        ...
        self.init()
        self.eps = self.params["eps"]
        self.__name__ = f"DeepPolicyMemory_{params['price']}_{params['memory']}"
        self.memory = self.params["memory"]
    ...
    def get_action(self,context):
        ...
        if len(df) < self.memory:
            ...
            tmp = df[["user_id","bike_id","accepted"]].sample(self.memory)
            
            count = tmp.groupby("accepted").user_id.count().min()
            
            try:
                tmp = pd.concat([tmp[tmp.accepted == 1].sample(count),tmp[tmp.accepted == 0].sample(count)])
            except ValueError:
                pdb.set_trace()
            
            X1,X2 =   tmp[["user_id","bike_id"]].values[:,0],tmp[["user_id","bike_id"]].values[:,1]
            y = tmp["accepted"].values
            
            
            
            
            self.model.fit([X1,X2],y,verbose=0,epochs=2)
